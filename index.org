...
def calendar_spread_1(stock):
    profit_call_bought = black_scholes_call(stock, 50, .01, .3, 1/12) - 1.98
...
#+begin_src python :session *calendar_spreads*
profit_1_same_vol = [calendar_spread_1(x) for x in range(30, 70)]
#+end_src
...
#+begin_export html
<iframe src="./figs/profit_same_vol.html" width="100%" height="100%" style="border:none;">
</iframe>
#+end_export
